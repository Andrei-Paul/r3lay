...
static struct mg_server *s_server;    // Set by start_mongoose()
...
*/
//else
      system("echo CONNECT requested. >> log.txt");
      return MG_FALSE;
...
*/
//else
      system("echo REPLY requested. >> log.txt");
      return MG_FALSE;

    case MG_REQUEST:
/*      if ((client = mg_connect2(s_server, s_remote_host, s_remote_port, 0)) != NULL) {
...
*/
//else
      system("echo REQUEST requested. >> log.txt");
      return MG_FALSE;
...
static void start_mongoose(int argc, char *argv[]) {
  s_argv = argv;
  if ((s_server = mg_create_server(NULL, ev_handler)) == NULL) {
...
static void set_options(char *argv[]) {
  ...
  if (!path_exists(get_option(options, "document_root"), 1)) {
    ...
    notify("Setting document_root to [%s]",
           mg_get_option(s_server, "document_root"));
  }
  ...
  for (i = 0; options[i] != NULL; i += 2) {
    const char *msg = mg_set_option(s_server, options[i], options[i + 1]);
    if (msg != NULL) {
      notify("Failed to set option [%s] to [%s]: %s",
             options[i], options[i + 1], msg);
      if (!strcmp(options[i], "listening_port")) {
        mg_set_option(s_server, "listening_port", s_default_listening_port);
  ...
  chdir(mg_get_option(s_server, "document_root"));
  ...
  // Add an ability to pass listening socket to mongoose
  {
    ...
    if (env != NULL && atoi(env) > 0 ) {
      mg_set_listening_socket(s_server, atoi(env));
    }
  }
#endif
